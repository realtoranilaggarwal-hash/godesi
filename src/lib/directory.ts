...
export const getCategoryTree = cache(async () => categoryTree());
...
const categoryCounts = cachedQuery("category-counts", TAXONOMY_TTL, async () => {
  const rows = await db.business.groupBy({
    by: ["categorySlug"],
    where: { status: "APPROVED", categorySlug: { not: null } },
    _count: { _all: true },
  });
  return rows.map((row) => [row.categorySlug as string, row._count._all] as const);
});
