...
import { OrientedTile } from "@/components/OrientedTile";
...
export function BusinessTile({
  ...
  const frame = `group relative flex h-full overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
  ...
  const overlay = (
    <>
      ...
      />
      {premium ? (
        <span className="absolute left-2 top-2 z-10 rounded-full bg-gradient-to-r from-amber-500 to-rose-500 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-white shadow">
          ⭐ Featured
        </span>
      ) : null}
    </>
  ...
  if (image) {
    return (
      <OrientedTile
        ...
        imageHeightClass={smallImage ? "h-20" : "h-32"}
        className={frame}
        overlay={overlay}
      >
        {body}
