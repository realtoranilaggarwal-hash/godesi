...
import { SALE_SIDE_LABELS, agentMoney, isAgentCard, splitList } from "@/lib/agents";
...
export default async function AgentDashboardPage() {
  ...
  const business = await db.business.findUnique({
    ...
    include: { agentProfile: { include: { sales: { orderBy: { soldOn: "desc" } } } } },
  });
    ...
    return (
      <Card>
        <p className="text-sm text-slate-600">
          Create your business card first, then come back to add your agent credentials.
        </p>
        <Link
    ...
    <div className="mx-auto max-w-3xl space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Service areas, licence, specialties, awards and closed sales — all shown on{" "}
        ...
        {isAgentCard(business.subcategorySlug) ? null : (
          <p className="mt-2 rounded-xl bg-amber-50 p-3 text-sm text-amber-900">
            Your card is not listed under Real Estate Agents yet, so this section stays
            hidden on the public page. Change the subcategory in{" "}
      ...
      <Card>
        <AgentProfileForm
          defaults={{
            ...
            totalSales:
              profile?.totalSalesMinor === undefined || profile?.totalSalesMinor === null
                ? ""
                : String(fromMinor(profile.totalSalesMinor)),
            avgPrice:
              profile?.avgPriceMinor === undefined || profile?.avgPriceMinor === null
                ? ""
      ...
      <Card>
        ...
        <p className="mb-3 text-sm text-slate-600">
          Add closed transactions you are allowed to publish. Buyers use these to judge
          whether you know their price range and area.
        </p>
        <AgentSaleForm />
