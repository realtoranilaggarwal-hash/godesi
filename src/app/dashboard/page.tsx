    ...
    await Promise.all([
      business
        ? db.analyticsEvent.count({
            where: { businessId: business.id, type: "PROFILE_VIEW" },
          })
        : 0,
      business
        ? db.analyticsEvent.count({
      ...
      business
        ? db.analyticsEvent.count({
            where: { businessId: business.id, type: "WHATSAPP_CLICK" },
          })
        : 0,
  ...
  return (
    <div className="flex gap-6">
      <div className="min-w-0 flex-1 space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Plan:{" "}
              <Badge tone={plan === "FREE" ? "slate" : "indigo"}>
                {PLANS[plan].name}
              </Badge>
        ...
        {searchParams.posted ? (
          <Alert tone="success">Requirement posted.</Alert>
        ) : null}
        {searchParams.upgraded ? (
          <Alert tone="success">
            You are now on the {searchParams.upgraded} plan.
          </Alert>
        ) : null}
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          ...
          <Stat
            ...
            hint={
              reviewCount ? `${rating.toFixed(1)} average` : "No ratings yet"
            }
          />
          <Stat
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {(adStats._sum.impressions ?? 0).toLocaleString("en-IN")}{" "}
                  impressions ·{" "}
                  {(adStats._sum.clicks ?? 0).toLocaleString("en-IN")} clicks
                  across {adStats._count._all} banner
                  {adStats._count._all > 1 ? "s" : ""}
                </p>
              </div>
        ...
        <Card>
          ...
          {myLeads.length ? (
            ...
            <p className="text-sm text-slate-500">
              You have not posted any requirements.
            </p>
          )}
        </Card>
      </div>
