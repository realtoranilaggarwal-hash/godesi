...
const couponSchema = z.object({
  ...
  scope: z.enum(["PLAN", "ADS", "TICKETS"]),
  discountKind: z.enum(["PERCENT", "FIXED"]),
  amount: z.coerce.number().int().min(1, "Discount must be more than zero"),
  ...
  try {
    ...
    await db.coupon.create({
      data: {
        ...
        currency: parsed.data.discountKind === "FIXED" ? parsed.data.currency : null,
...
export async function createEventCouponAction(
  ...
  try {
    ...
    const parsed = parseCoupon(formData);
    if (!parsed.success) return { error: parsed.error.issues[0].message };
    if (!parsed.data.eventId) return { error: "Choose which event the coupon is for." };
    if (parsed.data.discountKind === "PERCENT" && parsed.data.amount > 100) {
      return { error: "A percentage discount cannot be more than 100." };
    }
...
export async function toggleCouponAction(formData: FormData) {
  ...
  if (user.role !== "ADMIN" && coupon.createdById !== user.id) throw new Error("FORBIDDEN");
