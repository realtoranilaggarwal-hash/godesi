      ...
      <Card>
            ...
            {pendingMeetups.map((profile) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400">
                    {profile.city}
                    {profile.state ? `, ${profile.state}` : ""} · {profile.user.email} ·{" "}
        ...
        {reportedMeetups.length ? (
          <div className="mt-4 rounded-xl bg-rose-50 p-3">
            <h3 className="text-sm font-bold text-rose-900">Reported profiles</h3>
      ...
      <Card id="upi">
        ...
        <p className="mb-3 text-sm text-slate-600">
          {upiEnabled() ? (
            <>
              Buyers pay into{" "}
              <span className="font-mono font-semibold">{upiVpa()}</span> quoting
              a reference. Check the credit in your bank or PhonePe history, then
              approve — approving activates the plan for 30 days.
            </>
          ) : (
            <>
